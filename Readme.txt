...
- Plane, Bullet, and Fuel: Specialized derived classes for the player, projectiles, and collectibles.

Polymorphism in action: The Game class stores all enemies in a std::vector of Enemy pointers. By overriding the virtual move() and draw() methods, the game loop can update all different enemy types with a single, clean loop without knowing their specific concrete types.
